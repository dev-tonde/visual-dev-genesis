...
-- Create RLS policies for vitals (caregivers can manage)
...
CREATE POLICY "Caregivers can view patient vitals" ON public.vitals
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Caregivers can insert vitals" ON public.vitals
...
CREATE POLICY "Caregivers can view all patients" ON public.patients
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.users 
    WHERE users.id = auth.uid() 
...
CREATE POLICY "Caregivers can manage medications" ON public.medications
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.users 
    WHERE users.id = auth.uid() 
...
CREATE POLICY "Caregivers can manage alerts" ON public.alerts
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.users 
    WHERE users.id = auth.uid() 
...
CREATE POLICY "Caregivers can manage care notes" ON public.care_notes
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.users 
    WHERE users.id = auth.uid() 
