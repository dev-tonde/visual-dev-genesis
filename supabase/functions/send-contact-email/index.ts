...
const validateInput = (payload: unknown): { data?: ContactEmailRequest; errors: string[] } => {
  ...
  if (data.name.length < CONTACT_NAME_MIN_LENGTH || data.name.length > CONTACT_NAME_MAX_LENGTH) {
    errors.push(
      `Name must be between ${CONTACT_NAME_MIN_LENGTH} and ${CONTACT_NAME_MAX_LENGTH} characters.`
    );
  }
  ...
  if (
    ...
    errors.push(
      `Message must be between ${CONTACT_MESSAGE_MIN_LENGTH} and ${CONTACT_MESSAGE_MAX_LENGTH} characters.`
    );
  }
...
const createSubmission = async (data: ContactEmailRequest & { ip_hash: string | null }) => {
  ...
  if (error || !insertedSubmission) {
    throw new Error(
      `Contact submission insert failed: ${error?.message || 'Unknown insert error'}`
    );
  }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!resend) {
      console.warn('RESEND_API_KEY is not configured; contact emails were skipped', {
        submissionId,
      });
    } else {
    ...
    return jsonResponse(errorResponse, normalizedErrorMessage.includes('validation') ? 400 : 500);
  }
};
